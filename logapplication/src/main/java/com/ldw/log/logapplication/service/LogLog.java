...
import com.ldw.log.logapplication.App;
...
public class LogLog {
    ...
    public static void println(CharSequence tag, CharSequence url, CharSequence parms, CharSequence response) {
        try {
            App.getRemoteBinder().println(tag, url, parms, response);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private static void printlnTag(String tag, CharSequence childTag, CharSequence msg) {
        try {
            ...
            ILogRemoteAidlInterface remoteAidlInterface = App.getRemoteBinder();
